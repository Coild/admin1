...
function buttonHapusPabrik(p) { // console.log(p);
    var link = "formHapusPabrik" + p;
    Swal.fire({
        title: "Hapus?",
        text: "Yakin Ingin Dihapus?",
        icon: "warning",
...
function filecheckimg() {
    ...
    if (! allowedExtensions.exec(filePath)) {
        ...
        fileInput.value = "";
        return false;
    } else if (fileInput.files[0].size / 1024 / 1024 > 2) {
        Swal.fire({icon: "error", title: "Maaf", text: "Ukuran Terlalu Besar (Maksimal 2MB)"});
        fileInput.value = "";
        return false;
    }
}
